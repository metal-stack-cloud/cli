...
func (t *TablePrinter) AuditTable(data []*apiv1.AuditTrace, wide bool) ([]string, [][]string, error) {
	...
	header := []string{"TIME", "REQUEST-ID", "USER", "TENANT", "METHOD"}
	if wide {
		header = []string{"TIME", "REQUEST-ID", "USER", "TENANT", "PROJECT", "METHOD", "SOURCE-IP", "RESULT-CODE", "BODY"}
	}
	...
	for _, audit := range data {
		...
		if wide {
			rows = append(rows, []string{time, id, user, tenant, project, method, sourceIp, resultCode, body})
		} else {
			rows = append(rows, []string{time, id, user, tenant, method})
		}
	}
